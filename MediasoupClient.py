...
class IncommingVideoProcessor:
    def __init__(self, add_frame_callback_fnc, width, height, vision_matcher_base_url, loop=None, token="", rd="", d="", q="", lang="es"):
        ...
        self.done = False  # TODO: when done is True, the connection should be closed
        self.pictures = []
        ...
        # to the main asyncio loop thread-safely.
        def _dispatch_to_loop(cb, *args):
            try:
                if self.asyncio_loop and self.asyncio_loop.is_running():
                    self.asyncio_loop.call_soon_threadsafe(cb, *args)
                else:
                    # Fallback if no loop (e.g., tests) – run inline.
                    cb(*args)
            except Exception:
                logger.exception("Dispatch error")

        # Callback: report alive -> schedule on loop
        def _on_report_alive(alive):
            try:
                if alive:
                    ...
                    try:
                        ...
                        logger.error("Error occurred sending failure: %s", e)

                print(f"Callback: The Person is {'alive' if alive else 'not alive'}.")
                self.done = True
                print(f"Is this done: {self.is_this_done()}")
            except Exception:
                logger.exception("Unhandled error in _on_report_alive")

        self.liveness_server_client.set_report_alive_callback(
        ...
        self.liveness_server_client.set_take_picture_callback(
            lambda take_picture, frame: _dispatch_to_loop(_on_take_picture, take_picture, frame)
        )
    ...
    def report_alive_callback(self, alive):
        # Kept for compatibility if you call it directly; not registered directly anymore.
        try:
            if (alive):
                logger.debug("The person is alive")
                self._compare_local_pictures_with_reference()
            ...
            # FIXED formatting to avoid logging TypeError
    ...
    def process_frame(self, i_frame):
        if self.add_frame_callback_fnc is not None:
            ...
            # SEND ONLY: responses will arrive via image_callback -> dispatched to loop
    ...
    async def _async_compare_local_pictures_with_reference(self):
        try:
            ...
            # FIXED formatting to avoid logging TypeError
            logger.error("Exception...Error getting reference images: %s", str(e))
            reference_images = []
        ...
        if len(reference_images) > 0:
            ...
            average_distance = (total_distance / len(distances)) if distances else 1
    ...
    async def _async_face_match(self, image1, image2):
        ...
        if matcher_save_match_images:
            ...
            # Note: self.match_counter may not exist; keep original behavior
            image1_filename = os.path.join(IMAGE_DIR, f"match_{getattr(self, 'match_counter', 0)}_{timestamp}_image1.jpg")
            cv2.imwrite(image1_filename, image1)
            image2_filename = os.path.join(IMAGE_DIR, f"match_{getattr(self, 'match_counter', 0)}_{timestamp}_image2.jpg")
